...
window.onload = function() {
     ...
     * 発動するカードを選択する
     */
    const selectActivateCard = (effArray:effect[],text:string,cancel?:boolean) => {
        ...
        return new Promise<Card>(async(resolve, reject) => {
            ...
            tmpEff.targetCard = await openCardListWindow.select(cardlist,1,1,tmpEff,text,cancel);
    ...
    const TriggerQuickeEffect = async() =>{
        ...
        const canActiveEffects =(EffArray:effect[],time:Time[])=>{
            return EffArray.filter(eff =>{ 
                if(eff.MissTheTiming){
                    console.log(time.slice(-1));
                    return time.slice(-1).map(t => eff.actionPossible(t)).includes(true);
                }else {
                    return time.map(t => eff.actionPossible(t)).includes(true);
                };
            });
        };
        do{
            ...
            const AllQuick = (()=>{
                const tmpQuickArray :effect[] = [];
                [...game.defaultDeck].forEach((card,index,array)=>{
                    if((card instanceof SpellCard && card.spellType == "Quick") || card instanceof TrapCard){
                        if(game.turn == "MyTurn"){
                            if(card.location=="ST" && card.face=="DOWN" && card.setThisTurn){
                                return;
                            };
                        }else if(game.turn == "EnemyTurn"){
                            if(card.location=="HAND" || (card.location=="ST" && card.face=="DOWN" && card.setThisTurn)){
                                return;
                            };
                        };
                    };
            ...
            const　TriggerObj = (() => { 
                ...
                return {
                    ...
                    "PriAny_Quick":TriggerPrivateAny.concat(AllQuick)
                };
            })();
            ...
            do{
                ...
                if(PubForced.length >1){
                    const activeEffOrg = (await selectActivateCard(PubForced,"発動する効果を選択してください")).effect.filter(eff => 
            ...
            do{
                ...
                if(PubAny.length >1){
                    const selectedCard = await selectActivateCard(PubAny,"発動する効果を選択してください",true)
                    if(selectedCard){
            ...
            if(1<=game.chain.length && game.turn == "EnemyTurn"){
                do{
                    const PriAny_Quick = canActiveEffects(TriggerObj.PriAny_Quick,TriggerTime);
                    if(1<=PriAny_Quick.length){
                        const selectedCard = await selectActivateCard(PriAny_Quick,"チェーンして発動しますか？",true);
                        if(selectedCard){
                            const activeEffOrg = selectedCard.effect.filter(eff => 
                                    ["Trigger","Quick"].includes(eff.effType)  && canActiveEffects([eff],TriggerTime) 
                                ).pop();
                            const result :effect = {...activeEffOrg,targetCard:[],costCard:[]};
                            game.nowTime = new Time;
                            game.nowTime.effectActived.push({
                            ...
                            if (selectedCard.face=="DOWN"){
                                await cardFlip(selectedCard)
                            };
                            if(selectedCard instanceof SpellCard && selectedCard.location=="HAND"){
                                await moveCard.HAND.toBOARD(selectedCard,"ATK");
                            };
                            await animationChainEffectActivate(result);
                            await result.whenActive(result);
                            game.timeArray.push({...game.nowTime});
                            ...
                            TriggerObj.PriAny_Quick = TriggerObj.PriAny_Quick.filter(eff => eff !== activeEffOrg ); 
                        }else{
                            TriggerObj.PriAny_Quick = [];
                        };
                    };
                }while(0 < canActiveEffects(TriggerObj.PriAny_Quick,TriggerTime).length);
            };
            ...
            console.log("NO TriggerEffect")
                ...
                await (async () => {
                    for(let eff of game.chain){
                        if(
                            (eff.card instanceof SpellCard && (eff.card.spellType=="Normal"||eff.card.spellType=="Quick" )) ||
                            (eff.card instanceof TrapCard && (eff.card.trapType=="Normal"||eff.card.trapType=="Counter" ))
                        ){
        ...
        cardContainer.mouseEnabled = !(game.gameSet);
    };
        ...
        HAND:async(card:Card)=>{
            ...
            function handleHandMover(event) {
                ...
                if(card instanceof SpellCard){
                    if(canActiveEffects(card).length >0 && genCardArray({location:["ST"]}).length < 5 && game.turn=="MyTurn"){
                        disprayButtonArray.push(card.button.ACTIVATE.buttonContainer);
                    };
                }else if(card instanceof MonsterCard){
                    if(canActiveEffects(card).length >0){
                ...
                if(card instanceof MonsterCard && card.RuleSScondition() && game.turn=="MyTurn"){
                    disprayButtonArray.push(card.button.SS.buttonContainer);
                };
                if(card instanceof MonsterCard && JudgeNS(card) && game.turn=="MyTurn"){
                    disprayButtonArray.push(card.button.NS.buttonContainer);
                    disprayButtonArray.push(card.button.SET.buttonContainer);
                }else if((card instanceof SpellCard || card instanceof TrapCard) && genCardArray({location:["ST"]}).length < 5 && game.turn=="MyTurn"){
                    disprayButtonArray.push(card.button.SET.buttonContainer);
                };
     ...
     * 保持情報のリセット
     */
    const infomationReset = {
        leaveBoard : (card: Card)=>{
            if(card instanceof MonsterCard){
                card.NSed = false;
        ...
        BOARD:{
            toGY:async(card: Card) => {
                if(["ST","MO","FIELD"].includes(card.location)){
                    infomationReset.leaveBoard(card);
                    await LocationSetting(card,"GY")
                    await Animation.toGY(card); 
                };
            },
    ...
    const handSpellActivate = async(card: SpellCard) => {
        cardContainer.mouseEnabled = false;
        const Effect = card.effect.find(Eff => ["CardActived","Quick"].includes(Eff.effType))
    ...
    const GyEffActivate = async() => {
        ...
        const selectedCard = await selectActivateCard(AllGyIgnition,"発動する効果を選択してください",true);
        if(selectedCard){
    ...
    const BoardIgnitionActivate = async(card:Card) => {
        cardContainer.mouseEnabled = false;
        const Effect = card.effect.find(Eff => ["Ignition","Quick"].includes(Eff.effType))
        const ActivedEffect = {...Effect,targetCard:[],costCard:[]}
        game.nowTime = new Time;
        game.nowTime.effectActived.push({
     ...
     * セットした魔法罠発動
     */
    const fieldSpellTrapActivate =  async(card: SpellCard | TrapCard) => {
        cardContainer.mouseEnabled = false;
        const Effect = card.effect.find(Eff => ["CardActived","Quick"].includes(Eff.effType));
        const ActivedEffect = {...Effect,targetCard:[],costCard:[]}
        game.nowTime = new Time;
    ...
    const dealDamage = async(point:number)=>{
        ...
        if(game.enemyLifePoint < 1 && !(game.extensionMode) ){
            if(await (openYesNoWindow("ワンキル達成！さらに継続して回しますか？"))){
                game.extensionMode = true;
                return
            }else{
                game.gameSet = true;
                gameEND();
     ...
     * デッキから墓地へ送る
     */
    const deckToGrave = (target: Card[], shuffle: boolean) => {
        return new Promise<void>(async(resolve, reject) => {
            await (async () => {
                for(let card of target) {
                    if(game.DECK.includes(card)){
                        await moveCard.DECK.toGY(card);
                        game.nowTime.move.push({
                            card:card,
                            from:"DECK",
                            to:"GY"
    ...
    const canActiveEffects =(card:Card)=>{
        ...
        if(game.turn=="MyTurn"){
            return card.effect.filter(eff => 
                ["CardActived","Quick","Ignition"].includes(eff.effType) &&
                eff.actionPossible({}));
        }else if(game.turn=="EnemyTurn"){
            return card.effect.filter(eff => 
                eff.effType=="Quick" &&
     ...
     * フェイズ移行時のアニメーション
     */
    const shadPhase = async(phase:"DRAW PHASE"|"STANBY PHASE"|"MAIN PHASE"|"END PHASE"|"TURN END"|"ENEMY TURN") => {
    ...
    const gameStart = async()=>{
        ...
        await deckShuffle();
        await draw(5);
        [myLP,EnemyLP,numOfcardsContainer,resetButton,trunEndButton,gameEndButton].forEach(obj=>{
    ...
    const toEndPhase = async()=>{
        cardContainer.mouseEnabled = false;
        trunEndButton.mouseEnabled = false;
        trunEndButton.alpha = 0.5;
        const rightswornArray = genCardArray({category:["RIGHTSWORN"],location:["MO"],face:["UP"]});

        await shadPhase("END PHASE");
        await timeout(500)
        await (async () => {
    ...
    const reset = async()=>{
        ...
        game.payLPcost = true;
        game.gameSet = false;
        game.extensionMode = false;
        trunEndButton.mouseEnabled = true;
        trunEndButton.alpha = 1.0;
    ...
    interface effectSetting{
        ...
        CARDDESTRUCTION(arg:SpellCard): effect[];
        ...
        JUNKCOLLECTOR(arg:Card): effect[];
        JEINLIGHTSWORN(arg:Card): effect[];
        EHRENLIGHTSWORN(arg:Card): effect[];
        SOLAREXCHANGE(arg:SpellCard): effect[];
        CHARGEOFTHELIGHTBRIGADE(arg:SpellCard): effect[];
        HANDDESTRUCTION(arg:SpellCard): effect[];
        SPELLREPRODUCTION(arg:SpellCard): effect[];
        FOOLISHBURIAL(arg:SpellCard): effect[];
        MONSTERREINCARNATION(arg:SpellCard): effect[];
        RETURNFROMTHEDD(arg:TrapCard): effect[];
    };
...
墓地のカードをデッキトップに置く`);
        ...
        JUNKCOLLECTOR:(card:Card)=>{
            ...
            eff1.actionPossible = (time:Time) =>{
                const boolarray = [
                    eff1.range.includes(card.location),
                    card.face == "UP",
                    1 <= genCardArray({location:["GY"],cardType:["Trap"]}).filter(trap=>trap.effect[0].activateCondition()).length
                ];
                return boolarray.every(value => value==true)
            };
            eff1.whenActive = (eff :effect) => {
                return new Promise(async(resolve, reject) => {
                    const cardlist = genCardArray({location:["GY"],cardType:["Trap"]}).filter(trap=>trap.effect[0].activateCondition());
            ...
            eff1.whenResolve = (eff :effect) => {
                return new Promise<void>(async(resolve, reject) => {
                    ...
                    if(2<=game.DECK.length){
                        await deckToGrave(game.DECK.slice(-2),false);
                    }else{
                        await deckToGrave(game.DECK.slice(-(game.DECK.length)),false);
                    };
                    game.timeArray.push({...game.nowTime});
                    resolve();
                });
        ...
        EHRENLIGHTSWORN:(card:Card)=>{
            ...
            eff1.whenResolve = (eff :effect) => {
                return new Promise<void>(async(resolve, reject) => {
                    game.nowTime = new Time;
                    if(3<=game.DECK.length){
                        await deckToGrave(game.DECK.slice(-3),false);
                    }else{
                        await deckToGrave(game.DECK.slice(-(game.DECK.length)),false);
                    };
                    game.timeArray.push({...game.nowTime});
            ...
            eff1.whenResolve = (eff :effect) => {
                return new Promise<void>(async(resolve, reject) => {
                    ...
                    const decrearLevel = (()=>{
                        ...
                        //     return 1;
                        // };
                        if(pubZone.filter(c => c.cardName==status.Kuraz.cardName).length < 3){
                            return 6;
                        }else if(pubZone.filter(c => c.cardName==status.JunkCollector.cardName).length < 2){
                            return 5;
                        }else{
                            return 4;
                        };
        ...
        CARDDESTRUCTION:(card:SpellCard)=>{
            const eff1 = new effect(card);
            eff1.effType = "CardActived";
            eff1.range = ["HAND","ST"]
            eff1.actionPossible = (time:Time) =>{
        ...
        SOLAREXCHANGE:(card:SpellCard)=>{
            ...
            eff1.actionPossible = (time:Time) =>{
                const boolarray = [
                    JudgeSpellTrapActivateLoc(card),
                    1<=genCardArray({category:["RIGHTSWORN"],location:["HAND"]}).length,
                    4<=game.DECK.length 
                ];
                return boolarray.every(value => value==true)
            };
        ...
        CHARGEOFTHELIGHTBRIGADE:(card:SpellCard)=>{
            const eff1 = new effect(card);
            eff1.effType = "CardActived";
            eff1.range = ["HAND","ST"];
            eff1.actionPossible = (time:Time) =>{
                const boolarray = [
                    JudgeSpellTrapActivateLoc(card),
                    1<=genCardArray({category:["RIGHTSWORN"],location:["DECK"]}).length,
                    4<=game.DECK.length 
                ];
                return boolarray.every(value => value==true)
            };
        ...
        HANDDESTRUCTION:(card:SpellCard)=>{
            ...
            eff1.actionPossible = (time:Time) =>{
                const boolarray = [
                    JudgeSpellTrapActivateLoc(card),
                    2<=game.HAND.filter(c=>c!=card).length,
                    2<=game.DECK.length 
                ];
                return boolarray.every(value => value==true)
            };
        ...
        SPELLREPRODUCTION:(card:SpellCard)=>{
            const eff1 = new effect(card);
            eff1.effType = "CardActived";
            eff1.range = ["HAND","ST"];
            eff1.actionPossible = (time:Time) =>{
                const boolarray = [
                    JudgeSpellTrapActivateLoc(card),
                    2<=game.HAND.filter(c=>c!=card && c.cardType=="Spell").length,
                    1<=genCardArray({location:["GY"],cardType:["Spell"]}).length
                ];
                return boolarray.every(value => value==true)
            };
            eff1.whenActive = (eff :effect) => {
                return new Promise(async(resolve, reject) => {
                    eff.targetCard = await openCardListWindow.select(genCardArray({location:["HAND"],cardType:["Spell"]}),2,2,eff,"捨てるカードを2枚選択してください");
                    await discard(eff.targetCard);
                    const cardlist = game.GY.filter(card=>card instanceof SpellCard);
                    eff.targetCard = await openCardListWindow.select(cardlist,1,1,eff,"手札に加える魔法を1枚選択してください");
                    await animationEffectTarget(eff.targetCard);
        ...
        FOOLISHBURIAL:(card:SpellCard)=>{
            ...
            eff1.actionPossible = (time:Time) =>{
                const boolarray = [
                    JudgeSpellTrapActivateLoc(card),
                    1<=genCardArray({cardType:["Monster"],location:["DECK"]}).length
                ];
                return boolarray.every(value => value==true)
            };
            eff1.whenActive = (eff :effect) => {
            ...
            eff1.whenResolve = (eff :effect) => {
                return new Promise<void>(async(resolve, reject) => {
                    game.nowTime = new Time;
                    const cardlist = genCardArray({cardType:["Monster"],location:["DECK"]});
                    if(1<=cardlist.length){
                        eff.targetCard = await openCardListWindow.select(cardlist,1,1,eff,"墓地に送るカードを選択してください");
                        await deckToGrave(eff.targetCard,true);
                    };
                    game.timeArray.push({...game.nowTime});
                    resolve();
                });
        ...
        MONSTERREINCARNATION:(card:SpellCard)=>{
            const eff1 = new effect(card);
            eff1.effType = "CardActived";
            eff1.range = ["HAND","ST"];
            eff1.actionPossible = (time:Time) =>{
                const boolarray = [
                    JudgeSpellTrapActivateLoc(card),
                    1 <= game.HAND.filter(c=>c!=card).length,
                    1 <= genCardArray({location:["GY"],cardType:["Monster"]}).length
                ];
                return boolarray.every(value => value)
            };
            eff1.whenActive = (eff :effect) => {
                return new Promise(async(resolve, reject) => {
                    eff.targetCard = await openCardListWindow.select(game.HAND,1,1,eff,"捨てるカードを1枚選択してください");
                    await discard(eff.targetCard);
                    const cardlist = game.GY.filter(card=>card instanceof MonsterCard);
                    eff.targetCard = await openCardListWindow.select(cardlist,1,1,eff,"手札に加えるモンスターを1枚選択してください");
                    await animationEffectTarget(eff.targetCard);
                    resolve();
                });
            };
        ...
        MAGICALEXPLOSION:(card:TrapCard)=>{
            ...
            eff1.effType = "Quick";
            eff1.range = ["ST"];
            eff1.activateCondition = ()=>{
                return 1 <= genCardArray({location:["GY"],cardType:["Spell"]}).length && game.HAND.length == 0;
            };
            eff1.actionPossible = (time:Time) =>{
                const boolarray = [
                    eff1.range.includes(card.location),
    ...
    mainstage.setChildIndex(numOfcardsContainer,0);
    ...
    const updateText = new createjs.Text(" /Update 2020.07.28 チェーンキャンセル時のフリーズバグを修正。", "24px serif","black");
    ...
    bugReportButton.on("click", function(e){
        const url = "https://twitter.com/share?related=twitterapi%2Ctwitter&hashtags=JunkBladeSimulatorBugReport&text="+
    ...
    const deckRecipe :{json:Object,num:number}[] = [
        {json:status.JunkCollector, num:2},
        {json:status.Kuraz, num:3},
        {json:status.EhrenLightsworn, num:3},
        {json:status.JeinLightsworn, num:2},

        {json:status.ChargeOfTheLightBrigade, num:3},
        {json:status.SolarExchange, num:3},
        {json:status.HandDestruction, num:3},
        {json:status.CardDestruction, num:1},   
        
        {json:status.HiddenArmory, num:3},
        {json:status.PhenixBlade, num:3},
        {json:status.DDR, num:3},
        {json:status.Reasoning, num:1},

        {json:status.FoolishBurial, num:2},
        {json:status.MagicStoneExcavation, num:2},
        {json:status.SpellReproduction, num:2},
        {json:status.MonsterReborn, num:1},
        {json:status.MagicalExplosion, num:2},
        {json:status.ReturnFromTheDD, num:1}
    ...
    TweetButtonImg.on("click", function(e){
        const url = "https://twitter.com/share?url=https://tsd0313.github.io/ygo-JunkBlade/dist/&related=twitterapi%2Ctwitter&hashtags=JunkBladeSimulator&text="+
                    "ジャンクブレードシミュレータ"
                    window.open(url, null,"width=650, height=300, personalbar=0, toolbar=0, scrollbars=1, sizable=1")
    }, null, false);
    ...
    const openResultWindow = async(messageText :Text)=>{
        ...
        const tweetTextResult = (()=>{
            if(game.enemyLifePoint<1){
                return "ジャンクブレード先攻1キル成功！"+ (-game.enemyLifePoint+8000).toFixed()+"ダメージ！";
            }else{
                return "ジャンクブレード先攻1キル失敗・・・　"
            };
        })();
        const firstHand ="初手→" + game.firstHand.join('/') + "\n";
        const tweetURL = "https://twitter.com/share?url=https://tsd0313.github.io/ygo-JunkBlade/dist/&related=twitterapi%2Ctwitter&hashtags=JunkBladeSimulator&text="+
    ...
    const openHowtoWindow = ()=>{
        ...
        [textA,textB,textC].forEach((obj,i,a)=>{
